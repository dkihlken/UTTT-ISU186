package UTTT;
	...
	public class UTTT {
		...
		/*TODO 
		 *add tie conditions (full board)               (todo)
		 *add loop for correct input                    (done)
		 *add winning grid double placement rule        (todo)
		 *finish the full grid method and use it        (ignore)
		 */
			int x;  //Big Board
			int y;  //Little Board
			boolean p1ValGood = true;  // ends the x loop
			boolean p2ValGood = true;  // ends the y loop
			//Initial moves
		...
		/*//FINISH IT!!!!!
		public static boolean checkFullGrid(int [] C){
			return true;
		}*/
		
		//precondition:free move
		public static int doublePlacementX(){
			int x;
			boolean p1ValGood = true;  // ends the x loop
			do{
				System.out.println("I need a new x value");
				x = kb.nextInt();
				checkWin(BigBoard,x);
				if(x<=8 && x>=0 && BigBoard[x]==0){
					p1ValGood = false;
				}
				else{
					System.out.println("Invalid move. Please try again.");
		...
		//postcondition: returns the new x value
		
		public static int minMaxAIMoveChoice(){
			return 0;
		}
